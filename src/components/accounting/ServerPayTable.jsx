...
import { ServerPayRecord } from '@/api/entities';
import { useGlobalData } from '@/components/GlobalDataContext';

export default function ServerPayTable({ serverPayRecords, employees, isLoading }) {
  ...
  // Filter for unpaid employee records only
  const unpaidEmployeeRecords = useMemo(() => {
    return serverPayRecords.filter(
      record => record.payment_status === 'unpaid' && record.server_type === 'employee'
    );
  }, [serverPayRecords]);
  ...
  const payData = useMemo(() => {
    const employeePayData = employees.map(employee => {
      const employeeRecords = unpaidEmployeeRecords.filter(record =>
        String(record.server_id) === String(employee.id)
      );

      if (employeeRecords.length === 0) return null;

      const totalOwed = employeeRecords.reduce((sum, record) => sum + (record.total_amount || 0), 0);
  ...
  }, [unpaidEmployeeRecords, employees]);
    ...
    return (
      <Card className="border-0 shadow-sm bg-white">
        <CardContent className="p-12 text-center">
          ...
          <p className="text-slate-500">There are no outstanding payments for employees.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <>
      <Accordion type="multiple" className="space-y-4">
        {payData.map(({ server, records, totalOwed }) => (
          <AccordionItem key={server.id} value={String(server.id)} className="bg-white border border-slate-200 rounded-lg shadow-sm">
