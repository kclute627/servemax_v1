...
export default function EditEmailTemplateDialog({ open, onOpenChange, template }) {
  ...
  useEffect(() => {
    if (open && template) {
      ...
      setTemplateType(template.template_type || "attempt");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await entities.EmailTemplate.update(template.id, templateData);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the email template for SendGrid integration.
          </DialogDescription>
        </DialogHeader>
