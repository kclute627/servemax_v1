...
import { Plus, Receipt, Edit, Trash2, Copy } from "lucide-react";
...
export default function InvoiceTemplatesList() {
  ...
  const loadTemplates = async () => {
    setIsLoading(true);
    try {
      const invoiceTemplates = await entities.InvoiceTemplate.list();
  ...
  const handleCreate = () => {
    toast({
      title: "Coming Soon",
      description: "Invoice template editor is under development",
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Invoice Templates</h2>
          <p className="text-slate-600 mt-1">
            Create and manage invoice templates for companies to use
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card className="py-12">
          <CardContent className="text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-900 mb-2">No Invoice Templates Yet</h3>
